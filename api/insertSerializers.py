...
class InsertProfileSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = Profile
        fields = ['is_master', 'background_img', 'id', 'url', 'user','authority','is_seller','shop_name','tel','nickname','user_code','email','introducer','is_approved','image','real_name','gender','birthday','zipcode','prefecture','city','address1','address2','corporate_name','message_notification_phone','message_notification_mail','other_notification_mail','other_notification_phone','allowed_by_id','allowed_by_tel','word','salon_category','is_hidden','created','modified','payload', 'tel_code']
...
class InsertOrderSerializer(serializers.HyperlinkedModelSerializer):
    ...
    shipped_date = serializers.DateField(allow_blank=True, allow_null=True)
    class Meta:
        model = Order
